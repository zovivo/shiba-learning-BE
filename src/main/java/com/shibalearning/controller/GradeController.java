...
public class GradeController {
    ...
    public ResponseData create(@RequestBody GradeInput gradeInput){
        ...
        if (grade == null)
            return new ResponseData(Status.FAIL,"Khối đã tồn tại",null);
        return new ResponseData(Status.SUCCESS,"Tạo khối mới thành công",grade);
    }

    @GetMapping("search")
    public ResponseData search(@RequestParam int page, @RequestParam int size){
        return  new ResponseData(Status.SUCCESS,"Thành công",gradeService.search(page, size));
    }
